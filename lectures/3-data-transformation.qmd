...
- note that we didn't actually do anything to the values in the data. the values in the sleep duration and quality of sleep columns remain the same as before, and no new column was created either (stay tuned!)
...
:::{.column}
```{r max.lines = 3}
# same as gtex$Gene, but pipeable!
# eg. gtex |> .... |> pull(some_col) 
gtex |> pull(Gene)
```
