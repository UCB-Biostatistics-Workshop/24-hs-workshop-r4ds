...
```{r}
filter(sleep, `Quality of Sleep` >=8 & `Stress Level` <= 3)
```
...
- How many people with normal BMI have sleep apnea? (Note: The sleep dataset has two different values to represent normal BMI. You can also use parentheses to indicate order of logical conjunctions)
...
```{r}
arrange(sleep, `Stress Level`)
```

## Descending sort
- Use the desc function to arrange by descending values
```{r}
arrange(sleep, desc(`Stress Level`))
```
...
```{r}
arrange(sleep, `Stress Level`, `Physical Activity Level`)
```
